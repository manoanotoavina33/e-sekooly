...
exports.streamCashReceiptPdf = streamCashReceiptPdf;
...
function streamCashReceiptPdf(res, format, receipt) {
    const isThermal = format !== "A4";
    const doc = new pdfkit_1.default({ size: PAGE_SIZES[format], margin: isThermal ? 10 : 42 });
    ...
    if (isThermal) {
        ...
        if (receipt.description)
            doc.text(`Détail : ${receipt.description}`);
        ...
        doc.fillColor("#64748b").fontSize(9).font("Helvetica").text(`Reçu de caisse — ${receipt.schoolName ?? receipt.cashRegisterName}`, 92, 70);
        doc.moveDown(3);
...
//# sourceMappingURL=cashReceiptPdf.js.map